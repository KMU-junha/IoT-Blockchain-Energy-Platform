...
try:
    ...
    logging.info("INA219 센서 초기화 성공")
except Exception as e:
    ina = None
    SENSOR_AVAILABLE = False
    logging.error(f"센서 초기화 실패: {str(e)}")

# 전력 데이터 캐시 (스레드 안전)
power_cache = {'power': 0.0, 'timestamp': time.time()}
power_cache_lock = threading.Lock()

# Flask 앱 설정
flask_app = Flask(__name__)
HTML_PAGE = """
<!DOCTYPE html><html><head>
<title>전력 그래프</title><script src="https://cdn.jsdelivr.net/npm/chart.js"></script>
</head><body><h2>실시간 전력</h2>
<canvas id="powerChart" width="600" height="300"></canvas>
<script>
const ctx = document.getElementById('powerChart').getContext('2d');
const data = {labels: [], datasets: [{label: '전력 (W)', borderColor: 'blue', data: [], fill: false}]};
const config = {type: 'line', data: data, options: {responsive: true, scales: {
x: {title: {display: true, text: '시간'}}, y: {title: {display: true, text: 'W'}, beginAtZero: true, suggestedMin: 0, suggestedMax:0.01, ticks: {stepSize: 0.002}}}}};
const powerChart = new Chart(ctx, config);
function fetchData() {
fetch('/power_data').then(res => res.json()).then(json => {
const now = new Date(); data.labels.push(now.toLocaleTimeString());
data.datasets[0].data.push(json.power);
while (data.labels.length > 30) {data.labels.shift(); data.datasets[0].data.shift();}
powerChart.update();});}
...
@flask_app.route('/power_data')
def power_data():
    with power_cache_lock:
        return jsonify({"power": power_cache['power']})
...
def sensor_loop():
    ...
    while True:
        if ina:
            try:
                current = ina.current
                scaled = round(current*0.01, 5)
            except Exception as e:
                scaled = 0.0
                logging.error(f"센서 오류: {str(e)}")
        else:
            scaled = 0.0
       
        with power_cache_lock:
            power_cache = {'power': scaled, 'timestamp': time.time()}
        time.sleep(1)
...
# 네트워크 환경 설정
MY_IP = "192.168.100.12"  # 자신의 IP로 수정
MY_PORT = 8765
websockets_set = set()
...
async def sync_chain():
    ...
    await asyncio.gather(*(ws.send(msg) for ws in websockets_set if ws.open))
...
async def finalize_contract(contract_id, terms, seller_username):
    try:
        ...
        Thread(target=gpio_control, daemon=True).start()
